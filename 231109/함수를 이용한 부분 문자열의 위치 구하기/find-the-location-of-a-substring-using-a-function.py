...
def func(input_s, purpose):
    ...
    for i in range(len(input_s)):
        if(seq == 0 and input_s[i] == purpose[seq]): # 첫 문자열이 같은 경우
            idx = i
            seq += 1 # seq 1 증가
            if(seq == len(input_s)): # 만약 seq 값과 입력 문자열이 같다면(즉, 목적 문자열이 존재하면)
                break
            if(i == len(input_s) and seq < len(input_s)): # 끝까지 비교했는데 목적 문자열이 존재하지 않으면
                idx = -1
            ...
            if(input_s[i] == purpose[seq]): # 첫 목적 문자가 해당 문자열과 같은 경우
                idx = i # 인덱스 재설정
                seq += 1
            if(seq == len(input_s)): # 만약 인덱스 값과 입력 문자열이 같다면(즉, 목적 문자열 길이가 1이고 존재하면)
                break
            if(i == len(input_s) and seq < len(input_s)): # 끝까지 비교했는데 목적 문자열이 존재하지 않으면
                idx = -1
    return idx
